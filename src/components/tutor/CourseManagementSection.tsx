...
export default function CourseManagementSection() {
  ...
  const avgProgress = enrolledCourses.length > 0 
    ? enrolledCourses.reduce((sum, enrollment) => sum + (enrollment.progress_percentage || 0), 0) / enrolledCourses.length 
    : 0;
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {(enrolledCourses || []).map((enrollment) => (
                <Card key={enrollment.id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardHeader>
                    <CardTitle className="text-lg line-clamp-2">{enrollment.course_title || 'Course'}</CardTitle>
                    <p className="text-sm text-muted-foreground">Instructor: {enrollment.instructor_name || 'Unknown'}</p>
                  </CardHeader>
